...
class ChangeColor:
    ...
    ignoreChangeColor = Property.Bool(default = False)
    
    def Initialize(self, initializer):
        if not self.ignoreChangeColor:
    ...
    def onChangeColor(self, cEvent):
        if self.RandomColor:
            nuColor = self.RandomHue()
        elif not cEvent.Color:
            nuColor = self.RandomHue()
        else:
            nuColor = cEvent.Color
        #endif
        
        if self.ColorShifting:
            self.RandomShiftColor = nuColor
        else:
    ...
    def onUpdate(self, updateEvent):
        if self.ColorShifting:
            if self.DebugMode:
                print(self.Owner.Name, ": ChangeColor.onUpdate():")
