...
class MouseResponse:
    ...
    EventDelay = Property.Float(default = 0.0)
    EventData = Property.Level()
    
    DestroyOffHovering = Property.Bool(default = False)
    ...
    def setHover(self, e):
        ...
        if self.DestroyOffHovering:
            self.Owner.DestructionTimer.deactivate()
    ...
    def dispatchEvent(self, parameters = None):
        if not self.SendEvent == "":
            ResponseEvent = Zero.ScriptEvent()
            ResponseEvent.Sender = self.Owner
            ResponseEvent.Data = self.EventData
            
            if parameters:
                ResponseEvent.Data = parameters
            
            seq = Action.Sequence(self.Owner)
            Action.Delay(seq, self.EventDelay)
            Action.Call(seq, self.Space.DispatchEvent, (self.SendEvent, ResponseEvent))
            Action.Call(seq, self.Owner.DispatchEvent, (self.SendEvent, ResponseEvent))
            Action.Call(seq, self.Owner.DispatchUp, (self.SendEvent, ResponseEvent))
            #self.Space.DispatchEvent(self.SendEvent, Zero.ScriptEvent())
    ...
    def onExit(self, e):
        ...
        if self.DestroyOffHovering:
            self.Owner.DestructionTimer.activate()
            self.Owner.Destroy()
