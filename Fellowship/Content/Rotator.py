...
class Rotator:
    ...
    def onUpdate(self, Event):
        ...
        if self.Sway and self.Timer > self.SwayPeriod or self.Timer < 0:
            self.negator *= -1.0
            self.Timer = 0.0
        ...
        if self.RotateZ:
            ...
            #angle = self.negator * 2 * math.pi * (self.Timer/(self.SwayPeriod -1))
            self.Owner.Transform.Rotation = VectorMath.Quat.AxisAngle(vecAxis, angle * NormalizedTime);
